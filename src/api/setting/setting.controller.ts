		...
		try {
			const many = await this.settingService.many({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_MANY'] ],
					isRequired: true,
				}),
	...
	async one(payload) {
		try {
			const output = await this.settingService.one({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_ONE'] ],
					isRequired: true,
				}),
				relations: Validators.obj('relations', payload['relations']),
				select: Validators.obj('select', payload['select']),
	...
	async drop(payload) {
		try {
			await this.settingService.drop({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_DROP'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id'], {
					isRequired: true,
	...
	async dropMany(payload) {
		try {
			await this.settingService.dropMany({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_DROP_MANY'] ],
					isRequired: true,
				}),
	...
	async create(payload) {
		try {
			const output = await this.settingService.create({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_CREATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
	...
	async update(payload) {
		try {
			await this.settingService.update({
				user: Validators.token('accessToken', payload['accessToken'], {
					accesses: [ process['ACCESS_MAIL_SETTING_UPDATE'] ],
					isRequired: true,
				}),
				id: Validators.id('id', payload['id']),
